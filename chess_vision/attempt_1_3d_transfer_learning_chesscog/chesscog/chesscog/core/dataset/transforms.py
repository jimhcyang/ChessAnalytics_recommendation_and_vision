...
class Shear:
    ...
    def _shear(cls, img: Image, amount: float) -> Image:
        ...
        img = img.transform(img.size, Image.AFFINE, (1, -amount, 0, 0, 1, 0))
        img = ImageOps.flip(img)
        return img
...
class _HVTransform(ABC):
    ...
    def __init__(
        self,
        horizontal: typing.Union[float, tuple, None],
        vertical: typing.Union[float, tuple, None],
    ):
...
def build_transforms(cfg: CN, mode: Datasets) -> typing.Callable:
    ...
    if mode == Datasets.TRAIN:
        ...
        t.append(
            T.ColorJitter(
                brightness=transforms.COLOR_JITTER.BRIGHTNESS,
        ...
        t.append(Scale(transforms.SCALE.HORIZONTAL, transforms.SCALE.VERTICAL))
        t.append(
            Translate(transforms.TRANSLATE.HORIZONTAL, transforms.TRANSLATE.VERTICAL)
        )
    if transforms.RESIZE:
        t.append(T.Resize(tuple(reversed(transforms.RESIZE))))
    t.extend([T.ToTensor(), T.Normalize(mean=_MEAN, std=_STD)])
    return T.Compose(t)
