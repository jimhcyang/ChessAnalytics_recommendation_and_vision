...
from chesscog.core.dataset import (
    build_dataset,
    build_data_loader,
    Datasets,
    unnormalize,
...
def _csv(
    model: torch.nn.Module, agg: StatsAggregator, model_name: str, mode: Datasets
) -> str:
...
def evaluate(
    ...
    model = device(model)
    model.eval()
    datasets = {mode: build_dataset(cfg, mode) for mode in datasets}
    ...
    for mode, dataset in datasets.items():
        ...
        if find_mistakes:
            ...
            mistakes_file = output_folder / f"{model_name}_{mode.value}_mistakes.png"
            logger.info(f"Writing mistakes to {mistakes_file}")
            img.save(mistakes_file)
            groundtruth_file = (
                output_folder / f"{model_name}_{mode.value}_groundtruth.csv"
            )
...
def perform_evaluation(classifier: str):
    ...
    parser.add_argument(
        "--model",
        help=f"the model to evaluate (if unspecified, all models in 'runs://{classifier}' will be evaluated)",
        type=str,
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out", help="output folder", type=str, default=f"results://{classifier}"
    )
    parser.add_argument(
        "--find-mistakes",
        help="whether to output all misclassification images",
        dest="find_mistakes",
        action="store_true",
    )
    ...
    with output_csv.open("w") as f:
        models = (
            list(URI(f"runs://{classifier}").glob("*/*.pt"))
