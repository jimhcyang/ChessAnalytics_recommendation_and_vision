...
def _get_members(f: zipfile.ZipFile) -> typing.Iterator[zipfile.ZipInfo]:
    parts = [
        ...
        if not name.endswith("/") and not ".DS_Store" in name and not "__MACOSX" in name
    ]
...
def download_zip_folder_from_google_drive(
    file_id: str,
    destination: os.PathLike,
    show_size: bool = False,
    skip_if_exists: bool = True,
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        gdd.download_file_from_google_drive(
            file_id=file_id, dest_path=zip_file, overwrite=True, showsize=show_size
        )
        logger.info(f"Unzipping {zip_file} to {destination}")
        shutil.rmtree(destination, ignore_errors=True)
        with zipfile.ZipFile(zip_file, "r") as f:
...
# def download_zip_folder_from_google_drive(file_id, destination_folder):
#     URL = "https://drive.google.com/uc?export=download"
#     session = requests.Session()

#     # Make the initial request to get the virus scan warning page
#     response = session.get(URL, params={'id': file_id}, stream=True)

#     # Parse the HTML response to extract the confirmation token
#     soup = BeautifulSoup(response.content, 'html.parser')
#     token = soup.find('input', {'name': 'confirm'}).get(
#         'value') if soup.find('input', {'name': 'confirm'}) else None

#     if token:
#         print("Token found:", token)  # Debug: check token value
#         # Make another request with the confirmation token
...
#     path = os.path.join(destination_folder, "downloaded_file.zip")
