...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyse misclassified positions")
    parser.add_argument(
        "--results",
    ...
    parser.add_argument(
        "--dataset", help="the dataset to evaluate", type=str, default="test"
    )
    ...
    if (
        "fen_predicted" not in df.columns
        or "fen_actual" not in df.columns
        or "fen_predicted_is_valid" not in df.columns
    ):
        print(
    ...
    print(
        "  of which are invalid positions:",
