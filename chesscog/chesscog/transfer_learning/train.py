...
def _train_model(model_type: str) -> typing.Tuple[torch.nn.Module, CN]:
    ...
    yaml_file = (
        URI("config://transfer_learning") / model_type / f"{model_file.stem}.yaml"
    )
    ...
    train_model(cfg, run_dir, model, is_inception, model_file.stem, eval_on_train=True)


if __name__ == "__main__":
    argparse.ArgumentParser(
        description="Fine-tune the classifiers on the new dataset."
    ).parse_args()
